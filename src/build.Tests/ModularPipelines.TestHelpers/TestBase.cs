...
public class DummyModule : SimpleTestModule<IDictionary<string, object>?>
{
...
public abstract partial class TestBase
{
    ...
    public Task<T> RunModule<T>()
        where T : class, IModule => RunModule<T>(new TestHostSettings());
    ...
    public Task<T> RunModule<T>(TestHostSettings testHostSettings)
        where T : class, IModule => RunModule<T>(testHostSettings, null);

    /// <summary>
    /// Runs a single module with the specified test settings and custom services.
    /// </summary>
    /// <typeparam name="T">The module type to run.</typeparam>
    /// <param name="testHostSettings">Settings for configuring the test host.</param>
    ...
    public Task<(T, T2)> RunModules<T, T2>()
        where T : class, IModule
        where T2 : class, IModule => RunModules<T, T2>(new TestHostSettings());
        ...
        where T2 : class, IModule
    {
        return ExecuteModulesAsync<(T, T2)>(
            ...
            modules => (modules.OfType<T>().Single(), modules.OfType<T2>().Single())
        );
    }
    ...
    public Task<(T, T2, T3)> RunModules<T, T2, T3>()
        where T : class, IModule
        where T2 : class, IModule
        where T3 : class, IModule => RunModules<T, T2, T3>(new TestHostSettings());
    ...
    public Task<(T, T2, T3, T4)> RunModules<T, T2, T3, T4>()
        ...
        where T2 : class, IModule
        where T3 : class, IModule
        where T4 : class, IModule => RunModules<T, T2, T3, T4>(new TestHostSettings());
        ...
        where T4 : class, IModule
    {
        return ExecuteModulesAsync<(T, T2, T3, T4)>(
            testHostSettings,
            builder =>
                builder.Services.AddModule<T>().AddModule<T2>().AddModule<T3>().AddModule<T4>(),
            modules =>
                (
                    modules.OfType<T>().Single(),
                    modules.OfType<T2>().Single(),
    ...
    public async Task<(T T, IPipeline Pipeline)> GetService<T>(
        Action<IServiceCollection>? configureServices
    )
        where T : notnull
    {
    ...
    public async Task<(T T, IPipeline Host)> GetService<T>(
        Action<PipelineBuilder, IServiceCollection> configureServices
    )
        where T : notnull
    {
        var builder = TestPipelineHostBuilder.Create();
